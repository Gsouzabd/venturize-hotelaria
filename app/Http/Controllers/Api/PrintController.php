...
class PrintController extends Controller
{
    ...
    public function getPedidoForPrint(int $pedidoId, Request $request): JsonResponse
    {
        try {
            ...
            $dadosImpressao = [
                ...
                'empresa' => [
                    'nome' => $empresa?->nome_fantasia ?? $empresa?->razao_social ?? config('app.name', 'Venturize Hotelaria'),
                    'endereco' => $empresa?->cep,
     ...
     *
     * IMPORTANTE: esta rota é um mecanismo de reforço/redundância para pedidos que
     * ALGUÉM (o painel web) já pediu pra imprimir — ou seja, só retorna pedidos que
     * já têm uma ImpressaoPedido com status 'pendente'. Não é descoberta autônoma
     * de pedidos novos: um pedido com itens que nunca teve getPedidoForPrint()
     * chamado por ninguém nunca aparece aqui.
     */
    ...
    /**
     * Lista impressoras ativas configuradas (usado pelo PrintingAgent/sincronizador).
     */
    public function listarImpressoras(): JsonResponse
    {
        try {
            $impressoras = Impressora::ativas()
                ->ordenadas()
                ->get()
            ...
            return response()->json([
                'success' => true,
                'data' => $impressoras->map(function ($imp) {
                    return [
                        'name' => $imp->nome,
                        'ip' => $imp->ip,
                        'port' => $imp->porta ?? 9100,
                ...
                'message' => 'Impressoras listadas com sucesso',
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Erro ao buscar impressoras: '.$e->getMessage(),
                'error_code' => 'LIST_PRINTERS_ERROR',
            ], 500);
        }
    }
}
